...
float TEXT_SPEED = .6;
String STORY_TEXT = "episode vi"+delim+ " it is a period of civil war. rebel spaceships, "+delim+ "striking from a hidden base,  have won their first victory against"+delim+ " the evil Galactic Empire. During the battle, rebel spies "+delim+ " managed to steal secret plans to the Empire\'s ultimate weapon, the DEATH STAR, "+delim+ "an armored space station with enough power to destroy an entire planet. "+delim+ "Pursued by the Empire\'s sinister agents, Princess Leia races home "+delim + "aboard her starship, custodian of the stolen plans that "+delim + "can save her people and restore freedom to the galaxy...."+delim+delim+delim+"in order to fight the empire to shoot blaster bolts click with mouse"+delim+"use the mouse for direction and the space bar to accelerate"+delim+"good luck and may the force be with you...";
String SECOND_TEXT = "star"+"\n"+ "wars";
...
class Bullet extends Chara  {
  ...
  {
    damage(0.5);
    if(group==PLAYER) fill(0, 128, 255, 128);
...
void setup()
{
  ...
  playerLaser = new SoundFile (this, "heat-vision.mp3");
  enemyLaser = new SoundFile (this, "sf_laser_15.mp3");
  explosion = new SoundFile (this, "SFB-explosion2.mp3");
  fightmusic = new SoundFile (this, "Star Wars Episode 4 Soundtrack - TIE Fighter Attack.mp3");
  ...
  gameState = "INTRO";
   ...
   introFont = createFont("SF Distant Galaxy AltOutline",350);
   textAlign(CENTER);
 
  
  starwars = createFont("Star Jedi", 34);
  ...
  music = new SoundFile (this, "Star Wars Intro HD 1080p.mp3");
...
void drawStars() {
  ...
  int seed = int(random(1000000)); randomSeed(0);
  float range = 500.0;
